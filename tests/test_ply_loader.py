...
def test_load_synthetic_ply(tmp_path: Path) -> None:
    ...
    scene, _stats, _total = load_ply(ply_path)
...
def test_normalize_to_unit_extent(tmp_path: Path) -> None:
    ...
    scene, stats, _total = load_ply(ply_path)
    lo, hi = scene.bounds()
    extent = hi - lo
    center = (lo + hi) / 2.0
...
def test_max_gaussians_random_subsample(tmp_path: Path) -> None:
    ...
    sub_a, _, _ = load_ply(ply_path, max_gaussians=8, rng=rng_a)
    sub_b, _, _ = load_ply(ply_path, max_gaussians=8, rng=rng_b)
    sub_c, _, _ = load_ply(ply_path, max_gaussians=8, rng=rng_c)
